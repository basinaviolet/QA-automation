...
public class Version1CalcErrorTest {
    CalculatorVersion1 calculator;
    private int a, b;
    private float c;
    private char operation;

    public Version1CalcErrorTest(int a, int b, float c, char operation) {
        this.a = a;
        this.b = b;
        this.c = c;
    ...
    @Parameterized.Parameters (name = "{0} {3} {1} = AssertionError")
    ...
    @Test(expected = AssertionError.class, timeout = 40)
    public void testCheckingErrors() {
        calculator = new CalculatorVersion1(a, b);
        assertEquals(c, calculator.calculate(operation), 0);
    }
}
